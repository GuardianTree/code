...
def unet(T1_shape):
    ...
    conv_4=conv(merg4,f)
    _, H, W, C = conv_4.get_shape()
    Caps1_reshaped = layers.Reshape((H.value, W.value, 1, C.value))(conv_4)
    Caps2 = ConvCapsuleLayer(kernel_size=3,num_capsule=2, num_atoms=32, strides=1, padding='same', 
               routings=3,kernel_initializer='he_normal')(Caps1_reshaped)
    out_seg =(Length1(num_classes=2, seg=True, name='out_seg')(Caps2))

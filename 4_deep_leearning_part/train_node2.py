...
from  Network.Unet1_8 import unet
...
def data_generator(path1,path2,Label_path1,Label_path2,num,sample_size,size):
    while(True):
         for i in range(num):
...
def var_generator(path1,path2,Label_path1,Label_path2,num,sample_size,size):
    while(True):
         for i in range(num):
             dataA=loadmat(path1,path2,i)
             DataA=sample_data(dataA,sample_size,size)
             ...
             y = keras.utils.to_categorical(Y, num_classes=2)
             yield X1,y
def shape_change(A,len_data):
    CoorA = np.zeros((len_data), dtype=np.float64)
    (x,y)=A.shape

if __name__ == '__main__':
    path_xls='/storage/student22/net_hinge/mymodel/xlsx/sulc_16_std_U_SE_8.xlsx'
    filepath = "/storage/student22/net_hinge/mymodel/sulc_16_std_U_SE_8.h5"
    filepath2 = "/storage/student22/net_hinge/mymodel/sulc_16_std_save_U_SE_8.h5"#_e
    
    #filepath = "/storage/ZLL/code/mymodel/2add_16_std_U_SE_24_t.h5"#_ex
    #filepath2 = "/storage/ZLL/code/mymodel/2add_16_std_save_U_SE_24_t.h5"#_e
    #path_xls='/storage/ZLL/code/mymodel/xlsx/2add_16_std_U_SE_24_t.xlsx'
    #####  data_load ####a
    
    # T1
    train_1_l ='/storage/student22/3hinge_seg/1000/new_data/16/train/sulc/lh/'
    train_1_r ='/storage/student22/3hinge_seg/1000/new_data/16/train/sulc/rh/'
    ...
    #test_l ='/storage/student22/3hinge_seg/1000/new_data/16/test/area/lh/'
    #test_r ='/storage/student22/3hinge_seg/1000/new_data/16/test/area/rh/'
    ...
    #label_test_l ='/storage/student22/3hinge_seg/1000/new_data/16/test/label/lh/'
    #label_test_r ='/storage/student22/3hinge_seg/1000/new_data/16/test/label/rh/'
    vect=16
    #para
    xlen =900
    var_xlen =100
    ...
    #####load_data####
    Data=data_generator(train_1_l,train_1_r,label_l,label_r,xlen,sample_size,size)
    #print('Data.shape',Data.shape)
    var_Data=var_generator(test_1_l,test_1_r,label_test_l,label_test_r,
                          var_xlen,sample_size,size)
    ...
    history=model.fit_generator(Data,steps_per_epoch=xlen//batch_size,epochs=epoch,callbacks=callbacks_list,
            validation_data=var_Data,validation_steps=var_xlen//batch_size)
    model.save(filepath2)
    ### evaluate ###
    shape_change(A,len_data)
    BG1 = []
    hinge_List1 = []
    prec1 = []
    reca1 = []
    ...
    for i in range(num):
        ...
        print('test_labelA.shape',test_labelA.shape)

        predictionsA = model.predict(test_1_data)
        ...
        print(segmentationImage2A.shape)  #segmentationImage2A.shape=(576,576)
